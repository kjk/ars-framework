...
class iPediaProtocol(basic.LineReceiver):
	...
	def outputField(self, name, value):
		self.transport.write(name+fieldSeparator+value+lineSeparator)
	...
	def answer(self):
		...
		if (self.definitionRequested):
			...
			self.outputField(definitionField, str(len(testDefinition)))
			self.transport.write(testDefinition+lineSeparator)
